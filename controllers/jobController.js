...
module.exports = {
  ...
  /**
   * Renders the edit view of one single job.
   */
    ...
    Job.findOneAndUpdate(
      ...
      (err, job) => {
        if (err) {
          req.flash(
            "error",
            `There has been an error while updating the job offer: ${error.message}`
          );
          console.log(`Error updating job by ID: ${error.message}`);
        } else {
          req.flash(
            "success",
            `The job offer "${job.job_title}" has been successfully updated!`
          );
  ...
  deleteJob: (req, res) => {
    ...
    Job.findOneAndRemove({ _id: jobId }).then((job) => {
      ...
      User.findOneAndUpdate(
        ...
        .catch((error) => {
          req.flash(
            "error",
            `There has been an error while deleting the job offer: ${error.message}`
          );
